...
const BreakdownRow: React.FC<{
  ...
  const treatmentTargetAssessment = getReadingMedicationContext(
    reading,
    settings.takesAntihypertensiveMedication
  ) ? assessTreatmentTarget(reading.systolic, reading.diastolic, settings) : null;
...
const SessionCardItem: React.FC<{
  ...
  const treatmentTargetAssessment = getSessionMedicationContext(
    session.readings,
    settings.takesAntihypertensiveMedication
  ) ? assessTreatmentTarget(session.averageSystolic, session.averageDiastolic, settings) : null;
      ...
      <div className="session-main-row">
          ...
          {isMulti && (
            <div className="session-multi-toggle-block" onClick={(e) => e.stopPropagation()}>
              <button
                ...
                className="session-readings-toggle"
                title={isExpanded ? t('list.collapseReadings') : t('list.expandReadings')}
              >
                <ShieldCheck size={12} />
                <span>{session.readings.length}</span>
        ...
        {isDetailsExpanded && (
          <div className="session-details-expanded-row">
            {(settings.showInformationalLabels || safetyAlerts.length > 0) && <div className="session-block session-guideline-block">
              ...
              {((settings.showInformationalLabels && (culprit !== 'none' || healthAlerts.length > 0)) || safetyAlerts.length > 0) && (
                <div className="session-info-lines">
                  {settings.showInformationalLabels && culprit !== 'none' && (
                    <span className="session-info-line">{getCulpritLabel(culprit, category.direction, language)}</span>
                  )}
                  ...
                  {safetyAlerts.map((alert) => (
                    <span key={alert.key} className="session-info-line session-safety-text" role="alert">
            ...
            {settings.showInformationalLabels && treatmentTargetAssessment && (
              <div className="session-block session-target-block">
                <TreatmentTargetBadge assessment={treatmentTargetAssessment} />
              </div>
            )}

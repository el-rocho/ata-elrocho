  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (needsPulsePressureConfirmation(values)) {
      const valuesUnchanged =
        reading.systolic === sysNum &&
        reading.diastolic === diaNum &&
        reading.heartRate === hrNum;
      if (
        (valuesUnchanged && reading.pulsePressureWarningConfirmed === true) ||
        hasSimilarConfirmedReadingToday(readings, values, editedDateTime, reading.id)
      ) {
        saveReading(updated, true);
      } else {
        setPendingReading(updated);
      }
      return;
    }

...
import { Settings, X, ShieldAlert, Armchair, RotateCcw, User, Trash2, Globe, BookOpenCheck, Target, Info, ExternalLink, Repeat2, Gauge } from 'lucide-react';
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
              ...
              <div className={`medication-target-layout patient-profile-field-wide ${settings.takesAntihypertensiveMedication ? 'has-target' : ''}`}>
                <div className="patient-profile-field medication-setting-card">
                  ...
                  <div className="chip-options-row medication-options">
                    <button type="button" className={`chip-select medication-yes ${settings.takesAntihypertensiveMedication ? 'active' : ''}`} onClick={() => handleMedicationChange(true)}>{t('settings.medicationYes')}</button>
                    <button type="button" className={`chip-select medication-no ${!settings.takesAntihypertensiveMedication ? 'active' : ''}`} onClick={() => handleMedicationChange(false)}>{t('settings.medicationNo')}</button>
                  </div>
                </div>

                {settings.takesAntihypertensiveMedication && (
                  <div className="treatment-target-settings">
                    <div className="treatment-target-heading">
                      <Target size={18} />
                      <strong>{t('settings.targetTitle')}</strong>
                    </div>
                    <div className="chip-options-row treatment-target-modes">
                      <button type="button" className="btn-subtle-reset treatment-target-reset" onClick={handleResetTreatmentTarget}>
                        <RotateCcw size={13} /> {t('settings.targetGuideline')}
                      </button>
                    </div>
                    <div className="custom-target-grid">
                      {([
                        ['customTargetSystolicMin', 'settings.targetSystolicMin', 70, 250],
                        ['customTargetSystolicMax', 'settings.targetSystolicMax', 70, 250],
                        ['customTargetDiastolicMin', 'settings.targetDiastolicMin', 40, 150],
                        ['customTargetDiastolicMax', 'settings.targetDiastolicMax', 40, 150],
                      ] as const).map(([field, labelKey, min, max]) => (
                        <label key={field}>
                          <span>{t(labelKey)}</span>
                          <div className="target-number-input">
                            <input type="number" min={min} max={max} value={targetFieldValues[field]} onChange={(event) => handleCustomTargetChange(field, event.target.value)} onBlur={() => handleCustomTargetBlur(field)} />
                            <small>mmHg</small>
                          </div>
                        </label>
                      ))}
      ...
      {pendingMedicationValue !== null && (
        <div
          className="modal-overlay medication-context-overlay"
          onClick={(event) => {
            event.stopPropagation();
            if (!isUpdatingMedication) setPendingMedicationValue(null);
          }}
        >
          <div className="modal-content medication-context-dialog" onClick={(event) => event.stopPropagation()}>
            ...
            <div className="medication-context-choice">
              <strong>{t('settings.medicationKeepHistory')}</strong>
              <span>{t('settings.medicationKeepHistoryDesc')}</span>
            </div>
            <div className="medication-context-choice">
              <strong>{t('settings.medicationRecalculateHistory')}</strong>
              <span>{t('settings.medicationRecalculateHistoryDesc')}</span>
            </div>
            <div className="medication-context-actions">
              <button
                type="button"
                className="btn-secondary-large"
                disabled={isUpdatingMedication}
                onClick={() => setPendingMedicationValue(null)}
              >
                {t('settings.medicationChangeCancel')}
              </button>
              <button
                type="button"
                className="btn-recalculate-history"
                disabled={isUpdatingMedication}
                onClick={() => confirmMedicationChange(true)}
              >
                {t('settings.medicationRecalculateButton')}
              </button>
              <button
                type="button"
                className="btn-primary-large"
                disabled={isUpdatingMedication}
                onClick={() => confirmMedicationChange(false)}
              >
                {t('settings.medicationKeepButton')}
              </button>
            </div>
          </div>
        </div>

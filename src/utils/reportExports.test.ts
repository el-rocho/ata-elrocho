...
describe('history date filters', () => {
  it('uses clamped calendar months instead of fixed 30- and 90-day periods', () => {
    ...
    const sessionAt = (id: string, timestamp: Date): BloodPressureSession => ({
      ...discardedSession(),
      id,
    ...
    expect(filterSessionsByDateRange(sessions, { preset: '1month' }).map(({ id }) => id))
      .toEqual(['one-month-boundary']);
    expect(filterSessionsByDateRange(sessions, { preset: '3months' }).map(({ id }) => id))
      .toEqual(['one-month-boundary', 'older-than-one-month', 'three-month-boundary']);
  });
});
...
describe('clinical CSV export', () => {
  it.each(profiles)(
    ...
    (guidelineProfile, expectedCategory) => {
      ...
      expect(csv).toContain('# Tipo de archivo: informe CSV; no es una copia de seguridad');
  ...
  it('exports the applied therapeutic target and target status for medicated sessions', () => {
    const csv = buildCSVContent(
...
describe('clinical PDF content', () => {
  ...
  it('adds the existing target-status label to every medicated measurement', () => {
    const html = buildPDFMeasurementRowsHTML(
      [medicatedSession(), discardedSession()],
      'es',
      'esc-2024',
